...
const Townships = () => {
  ...
  const handleEdit = () => {
    const townshipToEdit = townships.find(t => t.guid === selected);
    if (townshipToEdit) {
      navigate('/admin/edit', {
        state: { entity: townshipToEdit, type: 'township' }
      });
    }
  };
  ...
  const handleDelete = async () => {
    if (selected) {
      try {
        ...
        console.error("Failed to delete township:", err);
      }
    }
  };

  const handleCreate = () => {
    const townshipTemplate = {
      name: '',
      provinceName: '',
    };
    navigate('/admin/create', {
      state: { createTemplate: townshipTemplate, type: 'township' }
    });
  };
  ...
  const visibleRows = useMemo(
    ...
    [filteredTownships, order, orderBy, page, rowsPerPage],
  );
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        {isLoading ? (
          ...
          <Box sx={{ p: 2 }}><Alert severity="error">Failed to fetch townships: {error.message}</Alert></Box>
        ) : (
          <>
                  ...
                  {visibleRows.map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="left">{row.provinceName || 'N/A'}</TableCell>
            ...
            <TablePagination
              ...
              component="div"
              count={filteredTownships.length}
